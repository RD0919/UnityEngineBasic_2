...
public class PlayerStatusUI : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        // 이컴포넌트를 가지는 GameObject가 가지는 컴포넌트들 중에 해당 타입을 찾아서 반환함
        ...
        //ex) RefreshHPBar
        //인라인함수는 접근제한자가 의미 없으므로 private 생략
        //구독하려는 대리자의 형식이 float 파라미터 1개와 void 반환이므로 void 및 float 타립 생략
        //인라인이므로 이름으로 함수 검색을 할 일이 없으므로 이름 생략
        //구현부 한줄이면 그다음은 반드시 함수 리턴이 일어나야하므로 함수 범위 지정 필요없으므로 중괄호 생략
        // 람다식 명시를 위해 => 추가
